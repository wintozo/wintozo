...
async function hashPassword(password: string): Promise<string> {
  ...
  const data = encoder.encode(password + "dev_test_salt_2026");
...
export default function DevPasswordPage() {
  ...
  const tempUsername = localStorage.getItem("dev_temp_username") || "";

  // Если нет сохранённого ника — редирект на первый шаг
  if (!tempUsername) {
    navigate("/dev/registration/username");
    return null;
  }
    ...
    try {
      ...
      if (existingUser) {
        if (existingUser.banned) {
          setError("Вы заблокированы в режиме тестера");
          setLoading(false);
          return;
        }
        ...
        const { error: insertError } = await supabase.from("wintozo_dev_users").insert({
          username: tempUsername,
          password_hash: passwordHash,
          avatar: tempUsername[0].toUpperCase(),
          description: "Тестер режима",
          created_at: new Date().toISOString(),
        });

        if (insertError) {
          if (insertError.code === "23505") {
            setError("Этот ник уже занят в режиме тестера");
          } else {
  ...
  const completeAuth = (username: string) => {
    ...
    localStorage.setItem("dev_avatar", username[0].toUpperCase());
    localStorage.setItem("dev_avatar_url", "");
    localStorage.setItem("dev_description", "Тестер режима");
    localStorage.setItem("dev_logged_in", "true");
    localStorage.removeItem("dev_temp_username");
    navigate("/dev/registration/select");
  };
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-yellow-300/80 text-xs mt-1">
                  Это ваш секретный код для входа в DEV режим. Без него вы не сможете вернуться.
                </p>
              </div>
            </div>
          </div>
