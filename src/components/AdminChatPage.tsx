...
export default function AdminChatPage() {
  ...
  useEffect(() => {
    ...
    // ОДНА ПОДПИСКА НА ВСЕ СООБЩЕНИЯ — НЕ ЗАВИСИТ ОТ selectedUser!
    ...
    // ОДНА подписка на presence (глобальная)
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-gray-900 dark:to-gray-800 flex transition-colors">
      ...
      {selectedUser ? (
        <div className="flex-1 flex flex-col h-screen">
          ...
          {showWarnBanner && (
            <div className="bg-red-50 dark:bg-red-900/30 border-b-2 border-red-200 dark:border-red-800 px-4 py-3">
          ...
          <form onSubmit={handleSendText} className="bg-white dark:bg-gray-800 border-t-2 border-purple-200 dark:border-gray-700 p-3 flex items-center gap-2 transition-colors">
            {sendingImage ? (
              ...
              <>
                ...
                <button
                  type="button"
                  onClick={() => selectedUser && openCall(selectedUser.username, 'audio')}
                  className="p-3 text-green-500 dark:text-green-400 rounded-xl hover:bg-green-50 dark:hover:bg-gray-700 shrink-0"
                >
