...
export default function NewChatPage() {
  ...
  const renderSidebar = (showBack: boolean) => (
    <>
      {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto">
          {pcGroups.length === 0 ? (
            <div className={`p-8 text-center ${theme.textSecondary}`}>
              ...
              <p className="text-sm">Вы не состоите ни в одной группе</p>
            </div>
          ) : (
            pcGroups.map((group) => (
              <button
                key={group.id}
                onClick={() => navigate(`/pc/chat/group/${group.id}`)}
                className={`w-full p-3 flex items-center gap-3 ${theme.hover} transition-all duration-200 text-left`}
              >
                <Avatar avatarUrl={group.avatar_url} avatarLetter={group.name[0]} size="md" />
                <div className="flex-1 min-w-0">
                  <p className={`font-medium ${theme.text} text-sm truncate`}>{group.name}</p>
                  <p className={`text-xs ${theme.textSecondary}`}>Группа</p>
                </div>
                <ChevronRight className={`w-4 h-4 ${theme.textSecondary}`} />
              </button>
            ))
  ...
  // Main layout — sidebar mode
  if (tab !== "chat" || !selectedUser) {
    return (
      <div className={`min-h-screen ${theme.bg} flex transition-colors duration-300`}>
        {/* Sidebar */}
        <div className={`w-80 ${theme.bgSecondary} border-r ${theme.border} flex flex-col transition-all duration-300`}>
          {renderSidebar(false)}
        </div>
        {/* Empty state */}
        <div className="flex-1 flex items-center justify-center">
          <div className={`text-center ${theme.textSecondary} transition-all duration-300`}>
            <MessageCircleIcon className="w-20 h-20 mx-auto mb-4 opacity-20" />
